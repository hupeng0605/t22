...
HBOOL KDialogP::DoMessage(HMessage* msg)
{
	...
	case VWM_SERVER_UPDATE_ITEM_STATUS:
		{
			//printf("VWM_SERVER_UPDATE_ITEM_STATUS = %d\r\n",msg->lParam);
			SetAlarmType((char)msg->lParam);
			UpdateHeadDevIcon();
		}
		break;
...
#if HIDE_VOL
	 m_pVolSize->Move(-120,0);
	 m_pTxtPM25 = new HCTextCtrl;
	 SAFECALL(m_pTxtPM25)->Create(280,16,80,32);
	 SAFECALL(m_pTxtPM25)->SetFontSize(22,22);
	 SAFECALL(m_pTxtPM25)->SetFont(GetRESingleton()->GetFont(0));
	 SAFECALL(m_pTxtPM25)->SetTxtColor(255, 255, 255);
	 m_pTxtPM25->SetBkgndN(GetHMIPath(_T("tpms\\1.hfp")));
	 //SAFECALL(m_pTxtPM25)->SetText(L"",5, 16,  RE_TS_NORMAL);
	 SAFECALL(m_pHeadGrpCtrl)->AddChild(m_pTxtPM25);
	 m_pTxtAlarm = new HCTextCtrl;
	 if(m_pTxtAlarm != NULL)
	 {
		 m_pTxtAlarm = new HCTextCtrl;
		 SAFECALL(m_pTxtAlarm)->Create(370,16,38,32);
		 SAFECALL(m_pTxtAlarm)->SetFontSize(22,22);
		 SAFECALL(m_pTxtAlarm)->SetFont(GetRESingleton()->GetFont(0));
		 SAFECALL(m_pTxtAlarm)->SetTxtColor(255, 255, 255);
		 m_pTxtAlarm->SetBkgndN(GetHMIPath(_T("tpms\\06-Setup-01_03.hfp")));
		 m_pTxtAlarm->SetShow(false);
		 //SAFECALL(m_pTxtPM25)->SetText(L"",5, 16,  RE_TS_NORMAL);
		 SAFECALL(m_pHeadGrpCtrl)->AddChild(m_pTxtAlarm);
	 }
#endif
...
void KDialogP::UpdatePM25(char cType)
{
...
void KDialogP::SetAlarmType(char cType)
{
		s_cAlarmType = cType;
}
#if HIDE_VOL
void KDialogP::UpdataTPMSAlarm()
{
	//printf("UpdataTPMSAlarm :%d\r\n",s_cAlarmType);
	DWORD dwValue = 0;
	HCRegSeting::GetRegValueInt(_T("SoftWare\\Hzcar\\App"), _T("TPMSAlarm"), dwValue);
	if(m_pTxtAlarm  !=NULL)
	{
		switch(dwValue)
		{
